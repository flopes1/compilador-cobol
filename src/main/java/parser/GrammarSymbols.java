package parser;


/**
 * This class contains codes for each grammar terminal
 * @version 2010-september-04
 * @discipline Compiladores
 * @author Gustavo H P Carvalho
 * @email gustavohpcarvalho@ecomp.poli.br
 */
public class GrammarSymbols {

	// Language terminals (starts from 0)
	public static final int ID = 1;
	
	public static final int IDENTIFICATOR_DIVISION = 2;
	
	public static final int DATA_DIVISION = 3;
	
	public static final int PROCEDURE_DIVISION = 4;
	
	public static final int DOT = 5;
	
	public static final int PROGRAMID = 6;
	
	public static final int INTEGER = 7;
	
	public static final int BOOLEAN = 8;
	
	public static final int BOOL = 9;
	
	public static final int NUMBER = 10;
	
	public static final int EXIT = 11;
	
	public static final int TO = 12;
	
	public static final int MOVE = 13;
	
	public static final int PERFORM = 14;
	
	public static final int USING = 15;
			
	public static final int UNTIL = 16;
	
	public static final int PIC = 17;
	
	public static final int COMP = 18;
	
	public static final int SECTION = 19;
	
	public static final int RETURN = 20;
	
	public static final int DISPLAY = 21;
	
	public static final int PLUS = 22;
	
	public static final int MINUS = 23;
	
	public static final int MULTIPLICATION = 24;
	
	public static final int DIVISION = 25;
	
	public static final int IF = 26;
	
	public static final int THEN = 27;
	
	public static final int ELSE = 28;
	
	public static final int END_IF = 29;
	
	public static final int END_WHILE = 30;
	
	public static final int END_PROC = 31;
	
	public static final int BREAK = 32;
	
	public static final int CONTINUE = 33;
	
	public static final int EOT = 1000;
	
}
